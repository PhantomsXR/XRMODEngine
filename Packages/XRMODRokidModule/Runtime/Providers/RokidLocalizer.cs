...
namespace Immersal.XR.Rokid
{
    ...
    public class RokidLocalizer : LocalizerBase
    {
        ...
        public void Init()
        {
            ...
            {
                FrameUpdateCallback = OnCameraDataUpdate;
                RokidExtensionAPI.RokidOpenXR_API_OpenCameraPreview(OnCameraDataUpdateCallWithImage);
            }
        ...
        public void OnCameraDataUpdate(byte[] data, ushort width, ushort height, long ts)
        {
            ...
            if (!Application.isEditor)
                RokidExtensionAPI.RokidOpenXR_API_GetHistoryCameraPhysicsPose(ts, position, rotation);
            ...
            float[] d = new float[5];
            if (!Application.isEditor)
                RokidExtensionAPI.RokidOpenXR_API_GetDistortion(d);
        ...
        public override async void Localize()
        {
            ...
            if (data.Bytes != null && data.Bytes.Length > 0)
            {
                ...
                if (m_PixelBuffer != IntPtr.Zero)
                {
                    ...
                    {
                        if (SolverType == SolverType.Lean)
                            return Immersal.Core.LocalizeImage(data.Width, data.Height, ref data.Intrinsics,
                                m_PixelBuffer,
                                rotArray);
        ...
        internal static void OnCameraDataUpdateCallWithImage(
            IntPtr _ptr,
            int _size,
            ushort _width,
            ushort _height,
            long _timestamp)
        {
            ...
            if (FrameUpdateCallback == null)
                return;
            FrameUpdateCallback(tmp_Data, _width, _height, _timestamp);
        }
    }
}
#endif