...
namespace Goap.Goap.Editor
{
    public class NodeElement : VisualElement
    {
            ...
            INode[] shownList = null)
        {
            ...
            this.Title = new Label($"{TypeReSolveHelper.ResolveType(graphNode.Action).GetGenericTypeName()}");
            this.Cost = new Label();
            ...
            foreach (var condition in graphNode.Conditions)
            {
                ...
                foreach (var connection in condition.Connections)
                {
                    ...
                    {
                        Debug.Log(
                            $"Skipping connection {TypeReSolveHelper.ResolveType(connection.Action).GetGenericTypeName()} because it's already shown in the list. Recursive connection detected!");
                        continue;
                    }

                    var connectionNode = new NodeElement(connection, bezierRoot, values,
                        new[] {connection}.Concat(list).ToArray());
            ...
            if (!Application.isPlaying)
            {
                if (graphNode.Action is IGoapAction goapAction)
                {
                    ...
                    this.Target.text =
                        $"Target: {TypeReSolveHelper.ResolveType(goapAction.Config.Target).GetGenericTypeName()}";
