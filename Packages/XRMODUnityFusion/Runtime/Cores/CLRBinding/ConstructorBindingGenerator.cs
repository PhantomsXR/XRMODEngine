    ...
    static class ConstructorBindingGenerator
    {
        ...
        internal static string GenerateConstructorWraperCode(this Type type, ConstructorInfo[] methods, string typeClsName, HashSet<MethodBase> excludes, List<Type> valueTypeBinders)
        {
            ...
            foreach (var i in methods)
            {
                ...
                sb.AppendLine("        {");
                sb.AppendLine("            UnityFusion.Runtime.Enviorment.AppDomain __domain = __intp.AppDomain;");
                if (param.Length != 0)
                ...
                for (int j = param.Length; j > 0; j--)
                {
                    ...
                    if (pt.IsValueType && !pt.IsPrimitive && valueTypeBinders != null && valueTypeBinders.Contains(pt))
                    {
                        ...
                        sb.AppendLine(string.Format("            if (UnityFusion.Runtime.Generated.CLRBindings.s_{0}_Binder != null) {{", clsName));

                        if (isMultiArr)
                            sb.AppendLine(string.Format("                UnityFusion.Runtime.Generated.CLRBindings.s_{1}_Binder.ParseValue(ref a{0}, __intp, ptr_of_this_method, __mStack, {2});", j, clsName, shouldFreeParam));
                        else
                            sb.AppendLine(string.Format("                UnityFusion.Runtime.Generated.CLRBindings.s_{1}_Binder.ParseValue(ref @{0}, __intp, ptr_of_this_method, __mStack, {2});", p.Name, clsName, shouldFreeParam));
                ...
                case ObjectTypes.StackObjectReference:
                    {
                        ...
                        if (p.ParameterType.IsValueType && !p.ParameterType.IsPrimitive && valueTypeBinders != null && valueTypeBinders.Contains(p.ParameterType))
                        {
                            ...
                            sb.AppendLine(string.Format("                        UnityFusion.Runtime.Generated.CLRBindings.s_{0}_Binder.WriteBackValue(__domain, ptr_of_this_method, __mStack, ref {1});", realClsName, p.Name));
                        ...
                        sb.AppendLine(@"            if(!isNewObj)
            {
                __ret--;");
                        ...
                        sb.AppendLine(string.Format("                    UnityFusion.Runtime.Generated.CLRBindings.s_{0}_Binder.WriteBackValue(__domain, __ret, __mStack, ref result_of_this_method);", clsName));
                        ...
                        sb.AppendLine(@"            if(!isNewObj)
            {
                ...
                if (type.IsValueType && valueTypeBinders != null && valueTypeBinders.Contains(type))
                {
                    ...
                    sb.AppendLine(string.Format("                UnityFusion.Runtime.Generated.CLRBindings.s_{0}_Binder.PushValue(ref result_of_this_method, __intp, __ret, __mStack);", clsName));
