...
{
    public class AudioHighPassFilterValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<AudioHighPassFilter>().ToList()
                    : convertedArray.Cast<AudioHighPassFilter>().ToArray());
            return true;
