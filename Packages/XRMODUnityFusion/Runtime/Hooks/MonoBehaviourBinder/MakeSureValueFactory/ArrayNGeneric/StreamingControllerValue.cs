...
{
    public class StreamingControllerValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<StreamingController>().ToList()
                    : convertedArray.Cast<StreamingController>().ToArray());
            return true;
