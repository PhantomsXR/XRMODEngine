...
{
    public class ScriptableObjectValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<ScriptableObject>().ToList()
                    : convertedArray.Cast<ScriptableObject>().ToArray());
            return true;
