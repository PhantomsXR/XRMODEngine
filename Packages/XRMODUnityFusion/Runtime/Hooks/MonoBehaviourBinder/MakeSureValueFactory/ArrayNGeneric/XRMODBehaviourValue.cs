...
{
    public class XRMODBehaviourValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<XRMODBehaviour>().ToList()
                    : convertedArray.Cast<XRMODBehaviour>().ToArray());
            return true;
    ...
    public class MonoBehaviourValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            if (!base.MakeSureValue(ref _fieldInfo, _target, _value)) return false;
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<MonoBehaviour>().ToList()
                    : convertedArray.Cast<MonoBehaviour>().ToArray());
            return true;
