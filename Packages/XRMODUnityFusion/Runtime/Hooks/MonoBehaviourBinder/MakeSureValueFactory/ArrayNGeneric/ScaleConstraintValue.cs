...
{
    public class ScaleConstraintValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<ScaleConstraint>().ToList()
                    : convertedArray.Cast<ScaleConstraint>().ToArray());
            return true;
