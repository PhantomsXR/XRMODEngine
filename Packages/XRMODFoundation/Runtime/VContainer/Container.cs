...
namespace VContainer
{
    ...
    public enum Lifetime
    {
        ...
        /// If parent and child have the same registration, the child will create its own instance.
        /// When a LifetimeScope is destroyed, objects with IDisposable implemented are called Dispose().
        /// <remarks>If the scene is alive and only LifetimeScope is destroyed,
        /// MonoBehaviours registered as Lifetime.Scoped are not automatically destroyed.
        /// If you want to destroy with LifetimeScope,
        /// make it a child transform of LifetimeScope or consider implement IDisposable.
        /// </remarks>
        /// </summary>
        Scoped
    }
    ...
    public sealed class ScopedContainer : IScopedObjectResolver
    {
        ...
        readonly ConcurrentDictionary<Registration, Lazy<object>> sharedInstances =
            new ConcurrentDictionary<Registration, Lazy<object>>();
        ...
        public object Resolve(Type type, object key = null)
        {
            ...
            throw new VContainerException(type,
                $"No such registration of type: {type} {(key == null ? string.Empty : $"with Key: {key}")}");
        }
        ...
        public void Inject(object instance)
        {
            ...
            {
                if (instance is ILTypeInstance tmp_ILTypeInstance)
                {
                    injector = InjectorCache.GetOrBuild(tmp_ILTypeInstance.Type.ReflectionType);
                }
                else if (instance is XRMODBehaviourAdapter.Adapter tmp_Adapter)
                {
                    injector = InjectorCache.GetOrBuild(tmp_Adapter.ILInstance.Type.ReflectionType);
                }
            }
            else
            {
                injector = InjectorCache.GetOrBuild(instance.GetType());
            }
#else
