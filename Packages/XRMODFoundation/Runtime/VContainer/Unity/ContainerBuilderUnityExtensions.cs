...
namespace VContainer.Unity
{
    ...
    public static class ContainerBuilderUnityExtensions
    {
        ...
        public static RegistrationBuilder RegisterComponent(
            this IContainerBuilder builder,
            Component component, Type _type)
        {
            RegistrationBuilder registrationBuilder = null;
            
            if (component is XRMODBehaviourAdapter.Adapter)
            {
                registrationBuilder = new ComponentRegistrationBuilder(component, _type).As(_type);
            }
            else
            {
                registrationBuilder = new ComponentRegistrationBuilder(component).As(_type);
            }

            // Force inject execution
            builder.RegisterBuildCallback(container => container.Resolve(_type));
            return builder.Register(registrationBuilder);
        }
