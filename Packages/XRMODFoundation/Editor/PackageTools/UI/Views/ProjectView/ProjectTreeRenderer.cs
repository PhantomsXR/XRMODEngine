...
namespace Phantom.XRMOD.XRMODPackageTools.Editor
{
    internal class ProjectTreeRenderer : TreeView
    {
        ...
        protected override void ContextClickedItem(int _id)
        {
            ...
            tmp_Menu.AddItem(new GUIContent("Delete"), false, () =>
            {
                if (!EditorUtility.DisplayDialog("Delete Project", "Do you want to delete this project? This action cannot be undone.",
                        "Delete",
                        CONST_CANCEL_NAME)) return;
                GenericMenuItemCommandData tmp_Data = new GenericMenuItemCommandData()
                {
                    CommandId = nameof(DeleteProjectCommand),
                    AllProjectsCache = allProjectsCache,
                    SelectedId = -1
                };
                foreach (int tmp_SelectionId in GetSelection())
        ...
        private static void DelayCallForEditingProject()
        {
            EditorApplication.delayCall += () =>
            {
                EditorApplication.delayCall = () =>
                {
                    ...
                    {
                        EditorNotificationCenter.Center.PostNotification(CONST_MODIFYTTEMPLATEMONOBINDER,
