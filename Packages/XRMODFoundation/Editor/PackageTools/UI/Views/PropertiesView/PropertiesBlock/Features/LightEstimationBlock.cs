    ...
    public class LightEstimationBlock : AbstractBlock
    {
        ...
        public override Rect DrawBlock(Rect _area)
        {
            lightEstimationBlockToggle =
                EditorGUILayout.BeginFoldoutHeaderGroup(lightEstimationBlockToggle, "Light Estimation", null,
            ...
            else if (lightEstimationBlockToggle)
            {
                ...
                {
                    property.LightShadowStrength = EditorGUILayout.Slider(new GUIContent("Shadow Strength"),
                        property.LightShadowStrength, 0.1f, 1f);
                }

                property.LightEstimationHDR =
                    EditorGUILayout.Toggle("Use HDR", property.LightEstimationHDR);

                if (property.LightEstimationHDR)
                {
                    EditorGUILayout.HelpBox(
                        "HDR mode only support android. If you are using it on iOS will be switch to face tracking algorithm automatic.",
                        MessageType.Warning);
                }
