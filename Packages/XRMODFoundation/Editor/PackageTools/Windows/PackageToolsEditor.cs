...
namespace Phantom.XRMOD.XRMODPackageTools.Editor
{
    public class PackageToolsEditor : EditorWindow
    {
        ...
        [MenuItem("Tools/XR-MOD/Package Tools Editor")]
        public static void ShowPackageToolsEditor()
        {
            ...
            var tmp_WindowPosition = PACKAGE_TOOLS_EDITOR.position;
            PACKAGE_TOOLS_EDITOR.position = new Rect(tmp_WindowPosition.position, new Vector2(600, 400));
//            PACKAGE_TOOLS_EDITOR.Close();
        ...
        private void OnDisable()
        {
            ...
            EditorNotificationCenter.Center.RemoveObserver(nameof(EditingProjectCommand));
            EditorNotificationCenter.Center.RemoveObserver(nameof(OpenProjectCommand));
            EditorNotificationCenter.Center.RemoveObserver(nameof(CreateProjectCommand));
            EditorNotificationCenter.Center.RemoveObserver(nameof(ExitEditingProjectCommand));
            ...
            {
                if (tmp_ProjectCache.DetailCacheData.Configures == null)
                {
                    var tmp_ProjectConfigures = ScriptableObject.CreateInstance<Configures>();
                    tmp_ProjectConfigures.name = nameof(Configures);
                    AssetDatabase.AddObjectToAsset(tmp_ProjectConfigures, tmp_ProjectCache);
        ...
        public void CreateGUI()
        {
            ...
            EditorNotificationCenter.Center.AddObserver(_data =>
                {
                    GenerateDetailViews();
        ...
        private static void LoadAllProjectCache()
        {
            ...
            foreach (ProjectCacheData tmp_ProjectCacheData in INSTANCE.ProjectCacheDataList)
            {
                var tmp_ProjectPath = tmp_ProjectCacheData.DetailCacheData.GetProjectPath();
                Utility.CreateRuntimeAssetReferenceDatabase(tmp_ProjectPath);
            }
        }
        ...
        private static void CheckProjectState()
        {
            ...
            {
                EditorNotificationCenter.Center.PostNotification(nameof(EditingProjectCommand), null);
            }
        }
