...
namespace Phantom.XRMOD.UIFramework.Runtime
{
    ...
    public class AnimationUI : MonoBehaviour
    {
        ...
        private bool isSequencesInitialized = false;
        ...
        {
            foreach (Sequence tmp_Sequence in AnimationSequence) tmp_Sequence.Init();
        }
        ...
        public void Play()
        {
            if (!isSequencesInitialized)
            {
                InitializeSequences();
                isSequencesInitialized = true;
            }

            Stop();
            coroutines.Add(StartCoroutine(PlayAnimation()));
        }
        ...
        public void PlayReversed()
        {
            ...
            Stop();
            coroutines.Add(StartCoroutine(PlayReversedAnimation()));
        }
        ...
        public void Stop()
        {
            foreach (Coroutine tmp_Coroutine in coroutines) StopCoroutine(tmp_Coroutine);
            coroutines.Clear();
        }
                ...
                if (sequence.SequenceType == Sequence.Type.Animation)
                {
                    ...
                    if (sequence.TargetType == Sequence.ObjectType.RectTransform)
                    {
                        if (sequence.TargetRtTask.HasFlag(Sequence.RtTask.AnchoredPosition))
                            coroutines.Add(StartCoroutine(TaskAnchoredPosition(
                                sequence.TargetComp.GetComponent<RectTransform>(),
                        ...
                        if (sequence.TargetRtTask.HasFlag(Sequence.RtTask.LocalEulerAngles))
                            coroutines.Add(StartCoroutine(TaskLocalEulerAngles(
                                sequence.TargetComp.GetComponent<RectTransform>(),
                        ...
                        if (sequence.TargetRtTask.HasFlag(Sequence.RtTask.LocalScale))
                            coroutines.Add(StartCoroutine(TaskLocalScale(
                                sequence.TargetComp.GetComponent<RectTransform>(),
                        ...
                        if (sequence.TargetRtTask.HasFlag(Sequence.RtTask.AnchorMax))
                            coroutines.Add(StartCoroutine(TaskAnchorMax(
                                sequence.TargetComp.GetComponent<RectTransform>(),
                        ...
                        if (sequence.TargetRtTask.HasFlag(Sequence.RtTask.AnchorMin))
                            coroutines.Add(StartCoroutine(TaskAnchorMin(
                                sequence.TargetComp.GetComponent<RectTransform>(),
                    ...
                    else if (sequence.TargetType == Sequence.ObjectType.Transform)
                    {
                        if (sequence.TargetTransTask.HasFlag(Sequence.TransTask.LocalPosition))
                            coroutines.Add(StartCoroutine(TaskLocalPosition(sequence.TargetComp.transform,
                        ...
                        if (sequence.TargetTransTask.HasFlag(Sequence.TransTask.LocalEulerAngles))
                            coroutines.Add(StartCoroutine(TaskLocalEulerAngles(sequence.TargetComp.transform,
                                sequence.LocalEulerAnglesStart, sequence.LocalEulerAnglesEnd, sequence.Duration,
                                sequence.EaseFunction
                            )));
                        if (sequence.TargetTransTask.HasFlag(Sequence.TransTask.LocalScale))
                            coroutines.Add(StartCoroutine(TaskLocalScale(sequence.TargetComp.transform,
                                sequence.LocalScaleStart, sequence.LocalScaleEnd, sequence.Duration,
                                sequence.EaseFunction
                            )));
                    }
                    else if (sequence.TargetType == Sequence.ObjectType.Image)
                    {
                        if (sequence.TargetImgTask.HasFlag(Sequence.ImgTask.Color))
                            coroutines.Add(StartCoroutine(TaskColor(sequence.TargetComp.GetComponent<Image>(),
                        ...
                        if (sequence.TargetImgTask.HasFlag(Sequence.ImgTask.FillAmount))
                            coroutines.Add(StartCoroutine(TaskFillAmount(sequence.TargetComp.GetComponent<Image>(),
                    ...
                    else if (sequence.TargetType == Sequence.ObjectType.CanvasGroup)
                    {
                        if (sequence.TargetCgTask.HasFlag(Sequence.CgTask.Alpha))
                            coroutines.Add(StartCoroutine(TaskAlpha(
                                sequence.TargetComp.GetComponent<CanvasGroup>(),
                    ...
                    else if (sequence.TargetType == Sequence.ObjectType.Camera)
                    {
                        if (sequence.TargetCamTask.HasFlag(Sequence.CamTask.BackgroundColor))
                            coroutines.Add(StartCoroutine(TaskBackgroundColor(
                                sequence.TargetComp.GetComponent<Camera>(),
                                sequence.BackgroundColorStart, sequence.BackgroundColorEnd, sequence.Duration,
                                sequence.EaseFunction
                            )));
                        if (sequence.TargetCamTask.HasFlag(Sequence.CamTask.OrthographicSize))
                            coroutines.Add(StartCoroutine(TaskOrthographicSize(
                                sequence.TargetComp.GetComponent<Camera>(),
                                sequence.OrthographicSizeStart, sequence.OrthographicSizeEnd, sequence.Duration,
                                sequence.EaseFunction
                            )));
                    }
                    else if (sequence.TargetType == Sequence.ObjectType.TextMeshPro)
                    {
                        if (sequence.TargetTextMeshProTask.HasFlag(Sequence.TextMeshProTask.Color))
                            coroutines.Add(StartCoroutine(TaskTextMeshProColor(
                                sequence.TargetComp.GetComponent<TMP_Text>(),
                                sequence.TextMeshProColorStart, sequence.TextMeshProColorEnd, sequence.Duration,
                                sequence.EaseFunction
                            )));
                        if (sequence.TargetTextMeshProTask.HasFlag(Sequence.TextMeshProTask.MaxVisibleCharacters))
                            coroutines.Add(StartCoroutine(TaskMaxVisibleCharacters(
                                sequence.TargetComp.GetComponent<TMP_Text>(),
                                (float) sequence.MaxVisibleCharactersStart,
                                (float) sequence.MaxVisibleCharactersEnd, sequence.Duration, sequence.EaseFunction
                            )));
                    }
                }
                ...
                if (sequence.SequenceType == Sequence.Type.Animation)
                {
                    ...
                    else if (sequence.TargetType == Sequence.ObjectType.Camera)
                    {
                        if (sequence.TargetCamTask.HasFlag(Sequence.CamTask.BackgroundColor))
                            coroutines.Add(StartCoroutine(TaskBackgroundColor(
                                sequence.TargetComp.GetComponent<Camera>(),
                                sequence.BackgroundColorEnd, sequence.BackgroundColorStart, sequence.Duration,
                                sequence.EaseFunction
                            )));
                        if (sequence.TargetCamTask.HasFlag(Sequence.CamTask.OrthographicSize))
                            coroutines.Add(StartCoroutine(TaskOrthographicSize(
                                sequence.TargetComp.GetComponent<Camera>(),
                                sequence.OrthographicSizeEnd, sequence.OrthographicSizeStart, sequence.Duration,
                                sequence.EaseFunction
                            )));
                    }
                    else if (sequence.TargetType == Sequence.ObjectType.TextMeshPro)
                    {
                        ...
                        if (sequence.TargetTextMeshProTask.HasFlag(Sequence.TextMeshProTask.MaxVisibleCharacters))
                            coroutines.Add(StartCoroutine(TaskMaxVisibleCharacters(
                                sequence.TargetComp.GetComponent<TMP_Text>(),
                                sequence.MaxVisibleCharactersStart, sequence.MaxVisibleCharactersEnd,
                                sequence.Duration, sequence.EaseFunction
                            )));
                    }
                }
                else if (sequence.SequenceType == Sequence.Type.Wait)
        ...
        IEnumerator TaskLocalPosition(Transform _trans, Vector3 _start, Vector3 _end, float _duration,
            Ease.Function _easeFunction)
        {
            float tmp_StartTime = Time.time;
            float tmp_Time = (Time.time - tmp_StartTime) / _duration;
            while (tmp_Time <= 1)
            {
                tmp_Time = Mathf.Clamp((Time.time - tmp_StartTime) / _duration, 0, 2);
                _trans.localPosition = Vector3.LerpUnclamped(_start, _end, _easeFunction(tmp_Time));
                yield return null;
            }

            _trans.localPosition = _end;
        }

        IEnumerator TaskLocalScale(Transform _trans, Vector3 _start, Vector3 _end, float _duration,
            Ease.Function _easeFunction)
        {
            float tmp_StartTime = Time.time;
            float tmp_Time = (Time.time - tmp_StartTime) / _duration;
            ...
            {
                tmp_Time = Mathf.Clamp((Time.time - tmp_StartTime) / _duration, 0, 2);
                _trans.localScale = Vector3.LerpUnclamped(_start, _end, _easeFunction(tmp_Time));
                yield return null;
            }

            _trans.localScale = _end;
        }

        IEnumerator TaskLocalEulerAngles(Transform _trans, Vector3 _start, Vector3 _end, float _duration,
            Ease.Function _easeFunction)
        {
            float tmp_StartTime = Time.time;
            float tmp_Time = (Time.time - tmp_StartTime) / _duration;
            ...
            {
                tmp_Time = Mathf.Clamp((Time.time - tmp_StartTime) / _duration, 0, 2);
                _trans.localEulerAngles = Vector3.LerpUnclamped(_start, _end, _easeFunction(tmp_Time));
                yield return null;
            }

            _trans.localEulerAngles = _end;
        }
        ...
        IEnumerator TaskBackgroundColor(Camera cam, Color start, Color end, float duration,
            Ease.Function easeFunction)
        {
            float startTime = Time.time;
            float t = (Time.time - startTime) / duration;
        ...
        IEnumerator TaskOrthographicSize(Camera cam, float start, float end, float duration,
            Ease.Function easeFunction)
        {
            float startTime = Time.time;
            float t = (Time.time - startTime) / duration;
        ...
        IEnumerator TaskTextMeshProColor(TMP_Text text, Color start, Color end, float duration,
            Ease.Function easeFunction)
        {
            float startTime = Time.time;
            float t = (Time.time - startTime) / duration;
        ...
        IEnumerator TaskMaxVisibleCharacters(TMP_Text text, float start, float end, float duration,
            Ease.Function easeFunction)
        {
            float startTime = Time.time;
            float t = (Time.time - startTime) / duration;
        ...
        public void PreviewStart()
        {
            ...
            foreach (Sequence sequence in AnimationSequence)
            {
                if (sequence.SequenceType == Sequence.Type.Animation)
                {
                    ...
                    {
                        if (sequence.TargetRtTask.HasFlag(Sequence.RtTask.AnchoredPosition))
                            sequence.TargetComp.GetComponent<RectTransform>().anchoredPosition =
                                sequence.AnchoredPositionStart;
                        if (sequence.TargetRtTask.HasFlag(Sequence.RtTask.LocalEulerAngles))
                            sequence.TargetComp.GetComponent<RectTransform>().localEulerAngles =
                    ...
                    {
                        if (sequence.TargetCamTask.HasFlag(Sequence.CamTask.BackgroundColor))
                            sequence.TargetComp.GetComponent<Camera>().backgroundColor =
                                sequence.BackgroundColorStart;
                        if (sequence.TargetCamTask.HasFlag(Sequence.CamTask.OrthographicSize))
                            sequence.TargetComp.GetComponent<Camera>().orthographicSize =
                    ...
                    else if (sequence.TargetType == Sequence.ObjectType.TextMeshPro)
                    {
                        ...
                        if (sequence.TargetTextMeshProTask.HasFlag(Sequence.TextMeshProTask.MaxVisibleCharacters))
                            sequence.TargetComp.GetComponent<TMP_Text>().maxVisibleCharacters =
            ...
            foreach (Sequence sequence in AnimationSequence)
            {
                            ...
                            if ((0 <= t - sequence.StartTime) && (t - sequence.StartTime < sequence.Duration))
                            {
                                ...
                                rt.anchoredPosition
                                    = Vector3.LerpUnclamped(sequence.AnchoredPositionStart,
                                        sequence.AnchoredPositionEnd,
                                        sequence.EaseFunction(
                                            Mathf.Clamp01((t - sequence.StartTime) / sequence.Duration)));
                            }
                ...
                else if (sequence.SequenceType == Sequence.Type.SetActive)
                {
                    void SetActive(float t)
                    {
                        ...
                        if (sequence.Target == null)
                        {
                            Debug.Log("Please assign Target for Sequence at " + sequence.StartTime.ToString() +
                                      "s");
                            return;
                        }
                ...
                //             if (t - sequence.StartTime >
                //                 -0.01f) //Nested conditional may actually more performant in this case
                //             {
                //                 sequence.IsDone = true;
                //                 sequence.Event?.Invoke();
                //             }
