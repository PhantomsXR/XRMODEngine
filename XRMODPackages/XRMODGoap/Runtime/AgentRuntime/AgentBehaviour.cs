    ...
    public class AgentBehaviour : MonoBehaviour, IMonoAgent
    {
        ...
        private void Start()
        {
            if (this.ActionProvider == null)
                throw new AgentException(
                    $"There is no ActionProvider assigned to the agent '{this.name}'! Please assign one in the inspector or through code in the Awake method.");
        }
        ...
        public void Initialize()
        {
            ...
            this.actionRunner = new ActionRunner(this,
                new AgentProxy(this.SetState, this.SetMoveState, (state) => this.ActionState.RunState = state,
                    this.IsInRange));
        ...
        [Obsolete(
            "Enable actions from within the action itself, or disable using actionProvider.GetActions<TAction>.ForEach((action) => action.Enable(IActionDisabler))")]
        ...
        [Obsolete(
            "Disable actions from within the action itself, or disable using actionProvider.GetActions<TAction>.ForEach((action) => action.Disable(IActionDisabler))")]
