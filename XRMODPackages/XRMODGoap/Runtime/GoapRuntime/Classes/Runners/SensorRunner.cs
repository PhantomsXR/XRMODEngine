    ...
    public class SensorRunner : ISensorRunner
    {
        ...
        )
        {
            ...
            foreach (var worldSensor in worldSensors)
            {
                ...
                this.sensors.TryAdd(TypeReSolveHelper.ResolveType(worldSensor.Key), worldSensor);
            }
            ...
            foreach (var targetSensor in targetSensors)
            {
                ...
                this.sensors.TryAdd(TypeReSolveHelper.ResolveType(targetSensor.Key), targetSensor);
            }
        ...
        private SensorSet CreateSet(IGoapAction action)
        {
            ...
            {
                set.Keys.Add(TypeReSolveHelper.ResolveType(condition.WorldKey));
            }
        ...
        private SensorSet CreateSet(INode node)
        {
            ...
            {
                var tmp_Key = TypeReSolveHelper.ResolveType(condition.WorldKey);

                set.Keys.Add(tmp_Key);

                if (this.sensors.TryGetValue(tmp_Key, out var sensor))
                {
                    set.AddSensor(sensor);
                }
            }
            ...
            foreach (var action in actions.Distinct())
            {
                ...
                {
                    var tmp_TargetType = TypeReSolveHelper.ResolveType(action.Config.Target);
                    set.Keys.Add(tmp_TargetType);
                    set.AddSensor(this.sensors[tmp_TargetType]);
                }
            }
