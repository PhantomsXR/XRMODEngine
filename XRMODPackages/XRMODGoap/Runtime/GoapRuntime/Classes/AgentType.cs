...
namespace Goap.Runtime
{
    public class AgentType : IAgentType
    {
            ...
            ISensorRunner sensorRunner, IGlobalWorldData worldData)
        {
            ...
            this.goals = goals;
            this.goalsLookup = goals.ToDictionary(TypeReSolveHelper.ResolveType);
        ...
        public IGoal ResolveGoal(Type _goalType)
        {
            if (this.goalsLookup.TryGetValue(_goalType, out var result))
                return result;

            throw new KeyNotFoundException($"No action found of type {_goalType}");
        }
        ...
        public List<TAction> GetActions<TAction>() where TAction : IGoapAction =>
            this.actions.OfType<TAction>().ToList();
