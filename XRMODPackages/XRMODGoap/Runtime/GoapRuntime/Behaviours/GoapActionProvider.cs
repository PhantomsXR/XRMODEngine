...
namespace Goap.Runtime
{
    public class GoapActionProvider : ActionProviderBase, IMonoGoapActionProvider
    {
        ...
        {
            if (this.AgentType == null)
                throw new GoapException(
                    $"There is no AgentType assigned to the agent '{this.name}'! Please assign one in the inspector or through code in the Awake method.");
        }
        ...
        /// </summary>
        /// <param name="_goalType"></param>
        /// <param name="_resolve"></param>
        public void RequestGoal(Type _goalType, bool _resolve = true)
        {
            this.ValidateSetup();
            var tmp_Request = this.GetRequestCache();
            tmp_Request.Goals.Clear();
            tmp_Request.Goals.Add(this.AgentType.ResolveGoal(_goalType));

            this.RequestGoal(tmp_Request, _resolve);
        }
        ...
        internal void RequestGoal(Type _goal1Type, Type _goal2Type, bool resolve = true)
        {
            this.ValidateSetup();

            var request = this.GetRequestCache();
            request.Goals.Clear();
            request.Goals.Add(this.AgentType.ResolveGoal(_goal1Type) as IGoal);
            request.Goals.Add(this.AgentType.ResolveGoal(_goal2Type) as IGoal);
        ...
        internal void RequestGoal(Type _goal1, Type _goal2, Type _goal3, bool _resolve = true)
        {
            this.ValidateSetup();

            var request = this.GetRequestCache();
            request.Goals.Clear();
            request.Goals.Add(this.AgentType.ResolveGoal(_goal1) as IGoal);
            request.Goals.Add(this.AgentType.ResolveGoal(_goal2) as IGoal);
            request.Goals.Add(this.AgentType.ResolveGoal(_goal3) as IGoal);

            this.RequestGoal(request, _resolve);
        }
        ...
        internal void RequestGoal(Type _goal1, Type _goal2, Type _goal3, Type _goal4, bool _resolve = true)
        {
            this.ValidateSetup();
            ...
            request.Goals.Add(this.AgentType.ResolveGoal(_goal2) as IGoal);
            request.Goals.Add(this.AgentType.ResolveGoal(_goal3) as IGoal);
            request.Goals.Add(this.AgentType.ResolveGoal(_goal4) as IGoal);
        ...
        internal void RequestGoal(Type _goal1, Type _goal2, Type _goal3, Type _goal4, Type _goal5, bool resolve = true)
        {
            this.ValidateSetup();
            ...
            request.Goals.Add(this.AgentType.ResolveGoal(_goal3) as IGoal);
            request.Goals.Add(this.AgentType.ResolveGoal(_goal4) as IGoal);
            request.Goals.Add(this.AgentType.ResolveGoal(_goal5) as IGoal);
        ...
        public void SetAction(IGoalResult result)
        {
            ...
            if (this.Logger.ShouldLog())
                this.Logger.Log(
                    $"Setting action '{TypeReSolveHelper.ResolveType(result.Action).GetGenericTypeName()}' for goal '{TypeReSolveHelper.ResolveType(result.Goal).GetGenericTypeName()}'.");
        ...
        public void SetDistanceMultiplierSpeed(float speed)
        {
            if (speed <= 0f)
                throw new GoapException(
                    "The speed value must be greater than 0. To disable the distance multiplier, use SetDistanceMultiplier(0f).");
        ...
        private void ValidateSetup()
        {
            ...
            if (this.Receiver == null)
                throw new GoapException(
                    $"There is no ActionReceiver assigned to the agent '{this.name}'! You're probably missing the ActionProvider on the AgentBehaviour.");
        }
        ...
        public List<IGoapAction> GetActions() => this.AgentType.GetActions();
        ...
        [Obsolete("Use AgentType instead")] public object GoapSet { get; set; }
