...
namespace Phantom.XRMOD.NetcodeModule.Runtime.NetworkDiscovery
{
        ...
        where TResponse : INetworkSerializable, new()
    {
        ...
        [SerializeField] long uniqueApplicationId;
        ...
        public long GetUniqueApplicationId()
        {
            ...
            var tmp_Value2 = (long) Random.Range(int.MinValue, int.MaxValue);
        ...
        async Task ReceiveBroadcastAsync()
        {
            ...
            try
            {
                ...
                reader.ReadNetworkSerializable(out TBroadCast _receivedBroadcast);

                if (ProcessBroadcast(udpReceiveResult.RemoteEndPoint, _receivedBroadcast, out TResponse tmp_Response))
                {
                    using var tmp_Writer = new FastBufferWriter(1024, Allocator.Persistent, 1024 * 64);
                    WriteHeader(tmp_Writer, MessageType.Response);

                    tmp_Writer.WriteNetworkSerializable(tmp_Response);
                    var tmp_Data = tmp_Writer.ToArray();

                    await m_Client.SendAsync(tmp_Data, tmp_Data.Length, udpReceiveResult.RemoteEndPoint);
                }
            }
            catch (Exception e)
            {
