...
namespace Phantom.XRMOD.NetcodeModule.Runtime.LagCompensation
{
    ...
    public class TrackedObject : MonoBehaviour
    {
        ...
        private void Start()
        {
            Assert.IsNotNull(LagCompensationManager.Singleton,
                $"{nameof(TrackedObject)} needs a {nameof(LagCompensationManager)}. Add a {nameof(LagCompensationManager)} to your scene.");
        ...
        public float AvgTimeBetweenPointsMs
        {
            get
            {
                if (m_Framekeys == null || m_Framekeys.Count == 0) return 0;
                    ...
                    return ((m_Framekeys.ElementAt(m_Framekeys.Count - 1) - m_Framekeys.ElementAt(0)) /
                            m_Framekeys.Count) * 1000f;
            }
        }
        ...
        internal void ReverseTransform(float secondsAgo)
        {
            ...
            transform.position = Vector3.Lerp(m_FrameData[previousTime].position, m_FrameData[nextTime].position,
                lerpProgress);
            transform.rotation = Quaternion.Slerp(m_FrameData[previousTime].rotation, m_FrameData[nextTime].rotation,
                lerpProgress);
        }
