...
namespace Immersal.AR
{
    ...
    public class ARMap : MonoBehaviour
    {
        ...
        [Space(10)] [Header("Map Metadata")] [SerializeField] [ReadOnly]
        private int m_MapId = -1;

        [SerializeField] [ReadOnly] private string m_MapName = null;
        [ReadOnly] public int privacy;
        [ReadOnly] public MapAlignment mapAlignment;
        [ReadOnly] public WGS84 wgs84;

        [Space(10)] [Header("Events")] public MapLocalizedEvent OnFirstLocalization = null;
        ...
        {
            double[] q = new double[]
                {this.mapAlignment.qw, this.mapAlignment.qx, this.mapAlignment.qy, this.mapAlignment.qz};
            double[] m = new double[9];
            ARHelper.DoubleQuaternionToDoubleMatrix3x3(out m, q);

            double[] mapToEcef = new double[]
            {
                this.mapAlignment.tx, this.mapAlignment.ty, this.mapAlignment.tz, m[0], m[1], m[2], m[3], m[4], m[5],
                m[6], m[7], m[8], this.mapAlignment.scale
            };
        ...
        public virtual async Task<int> LoadMap(byte[] mapBytes = null, int mapId = -1)
        {
            ...
            if (mapBytes != null)
            {
                Task<int> t = Task.Run(() => { return Immersal.Core.LoadMap(mapBytes); });
            ...
            if (this.mapId > 0 && m_ARSpace != null)
            {
                ...
                ARSpace.RegisterSpace(root, this, transform.localPosition, transform.localRotation,
            ...
            if (Application.isEditor)
            {
                if (mapFile != null)
                {
                    try
                    {
                        ...
                        string jsonFilePath = Path.Combine(destinationFolder,
                            string.Format("{0}-metadata.json", mapFile.name));
                        if (File.Exists(jsonFilePath))
                        {
                            MetadataFile metadataFile =
                                JsonUtility.FromJson<MetadataFile>(File.ReadAllText(jsonFilePath));
        ...
        private void OnRenderObject()
        {
            if (IsRenderable() && m_Material != null)
            {
                ...
                if (renderAs3dPoints)
                {
                    ...
                    m_Material.SetFloat("_PointSize",
                        Mathf.Lerp(0.002f, 0.14f, Mathf.Max(0, Mathf.Pow(pointSize, 3f))));
                }
                else
                {
