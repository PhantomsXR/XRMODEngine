...
namespace Phantom.XRMOD.XRMODAvatar.Runtime.XR
{
    public class JointBasedHandForAvatar : MonoBehaviour
    {
        ...
        protected XRHandJointID[] m_FingerStartJointIds =
        {
            ...
            try
            {
                ...
                for (int i = 0; i < handFidelityOptions.Length; i++)
                {
                    ...
                    for (int j = 0; j < handFidelityOptions[i].FingerJoints.Length; j++)
                    {
                        ...
                        handFidelityOptions[i].FingerJoints[j].JointTransformReferences =
                        ...
                        handFidelityOptions[i].FingerJoints[j].JointTransformReferences =
                            GetFingerJoints(m_FingerNames[j], startDepth, jointDepth, m_FingerStartJointIds[j]);
                    }

                    //Get extra fingers as mittens
                    if (i == 2)
                    {
                        handFidelityOptions[i].FingerJoints[2].JointTransformReferences
                            .AddRange(GetFingerJoints(m_FingerNames[3], 1, 3, m_FingerStartJointIds[3]));
                        handFidelityOptions[i].FingerJoints[2].JointTransformReferences
                            .AddRange(GetFingerJoints(m_FingerNames[4], 1, 3, m_FingerStartJointIds[4]));
                    }
                }
            }
            catch (Exception e)
        ...
        List<JointToTransformReference> GetFingerJoints(string FingerName, int startDepth, int jointDepth,
...
#if UNITY_EDITOR && USE_XR_HAND
    [CustomEditor(typeof(JointBasedHandForAvatar))]
    public class HandCurlerEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            DrawDefaultInspector();
            JointBasedHandForAvatar myScript = (JointBasedHandForAvatar) target;
