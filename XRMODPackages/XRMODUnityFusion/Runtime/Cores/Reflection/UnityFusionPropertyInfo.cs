...
{
    public class UnityFusionPropertyInfo : PropertyInfo
    {
        ...
        public UnityFusionPropertyInfo(Mono.Cecil.PropertyDefinition definition, ILType dType)
        {
            this.definition = definition;
            this.dType = dType;
            appdomain = dType.AppDomain;
            parameters = new UnityFusionParameterInfo[definition.Parameters.Count];
            for (int i = 0; i < definition.Parameters.Count; i++)
            {
                var pd = definition.Parameters[i];
                var parameterType = dType.AppDomain.GetType(pd.ParameterType, null, null);
                parameters[i] = new UnityFusionParameterInfo(pd, parameterType, this, appdomain);
            }
        }
        ...
        public UnityFusion.Mono.Cecil.TypeReference Definition
        {
            get
            {
