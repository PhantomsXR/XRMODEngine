...
namespace UnityFusion.Reflection
{
    public class UnityFusionType : Type
    {
        ILType type;
        Runtime.Enviorment.AppDomain appdomain;
        Attribute[] customAttributes;
        Type[] attributeTypes;
        UnityFusionFieldInfo[] fields;
        UnityFusionPropertyInfo[] properties;
        UnityFusionMethodInfo[] methods;
        ...
        public UnityFusionType(ILType t)
        {
            type = t;
        ...
        void InitializeCustomAttribute()
        {
            ...
            for (int i = 0; i < type.TypeDefinition.CustomAttributes.Count; i++)
            {
                ...
                try
                {
                    ...
                    attributeTypes[i] = at.ReflectionType is UnityFusionWrapperType ? at.TypeForCLR : at.ReflectionType;
        ...
        void InitializeProperties()
        {
            if (type.TypeDefinition == null)
            {
                properties = new UnityFusionPropertyInfo[0];
                return;
            }

            int cnt = type.TypeDefinition.HasProperties ? type.TypeDefinition.Properties.Count : 0;
            properties = new UnityFusionPropertyInfo[cnt];
            for (int i = 0; i < cnt; i++)
            {
                Mono.Cecil.PropertyDefinition pd = type.TypeDefinition.Properties[i];
                UnityFusionPropertyInfo pi = new UnityFusionPropertyInfo(pd, type);
                properties[i] = pi;
                if (pd.GetMethod != null)
                {
                    pi.Getter = type.GetMethod(pd.GetMethod.Name,
                        pd.GetMethod.Parameters.Select(p => type.AppDomain.GetType(p.ParameterType, type, null))
                            .ToList(), null) as ILMethod;
                }

                if (pd.SetMethod != null)
                {
                    pi.Setter = type.GetMethod(pd.SetMethod.Name,
                        pd.SetMethod.Parameters.Select(p => type.AppDomain.GetType(p.ParameterType, type, null))
                            .ToList(), null) as ILMethod;
                }
            }
        }
        ...
        void InitializeMethods()
        {
            ...
            this.methods = new UnityFusionMethodInfo[methods.Count];
            for (int i = 0; i < methods.Count; i++)
            {
                this.methods[i] = (UnityFusionMethodInfo) ((ILMethod) methods[i]).ReflectionMethodInfo;
            }
        }
        ...
        public override Type MakeGenericType(params Type[] typeArguments)
        {
            if (ILType.TypeReference.HasGenericParameters)
            {
                ...
                for (int i = 0; i < ga.Length; i++)
                {
                    ...
                    if (typeArguments[i] is UnityFusionType)
                        ga[i] = new KeyValuePair<string, IType>(key, ((UnityFusionType) typeArguments[i]).ILType);
                    else
        ...
        void InitializeFields()
        {
            ...
            fields = new UnityFusionFieldInfo[type.TotalFieldCount + staticCnt];
            for (int i = 0; i < type.TotalFieldCount; i++)
            {
                Mono.Cecil.FieldDefinition fd;
                var t = type.GetField(i, out fd);
                fields[i] = new UnityFusionFieldInfo(fd, this, i, t);
            }

            for (int i = type.TotalFieldCount; i < type.TotalFieldCount + staticCnt; i++)
            {
                fields[i] = new UnityFusionFieldInfo(type.StaticFieldDefinitions[i - type.TotalFieldCount], this, true,
        ...
        {
            get { return typeof(UnityFusionType).Assembly; }
        }
        ...
        {
            get { return type.TypeDefinition.Namespace; }
        }

        public override Type UnderlyingSystemType
        {
            get { return typeof(ILTypeInstance); }
        }
        ...
        {
            IType type;
            if (c is UnityFusionWrapperType)
            {
                type = ((UnityFusionWrapperType) c).CLRType;
            }
            else if (c is UnityFusionType)
            {
                type = ((UnityFusionType) c).ILType;
            }
            else
        ...
        public override FieldInfo GetField(string name, BindingFlags bindingAttr)
        {
            ...
            if (BaseType != null && BaseType is UnityFusionWrapperType)
            {
        ...
        public override MethodInfo[] GetMethods(BindingFlags bindingAttr)
        {
            ...
            if ((bindingAttr & BindingFlags.DeclaredOnly) != BindingFlags.DeclaredOnly)
            {
                if (BaseType != null /*&& (BaseType is UnityFusionWrapperType || BaseType is UnityFusionType)*/)
                {
        ...
        public override bool IsDefined(Type attributeType, bool inherit)
        {
            ...
            for (int i = 0; i < customAttributes.Length; i++)
            {
                if (attributeTypes[i].Equals(attributeType))
                    return true;
                if (attributeTypes[i] is UnityFusionWrapperType iwt)
            ...
            CallingConventions callConvention, Type[] types, ParameterModifier[] modifiers)
        {
            ...
            {
                if (types[i] is UnityFusionType)
                    param.Add(((UnityFusionType) types[i]).type);
                else
                {
        ...
        {
            return o is UnityFusionType ? ((UnityFusionType) o).type == type : false;
        }
#endif
        ...
        {
            get { return type.HasGenericParameter || type.GenericArguments != null; }
        }
        ...
        {
            get { return type.HasGenericParameter; }
        }
    }
}