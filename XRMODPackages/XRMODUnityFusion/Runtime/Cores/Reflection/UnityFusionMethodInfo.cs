...
{
    public class UnityFusionMethodInfo : MethodInfo
    {
        ILMethod method;
        UnityFusionParameterInfo[] parameters;
        Mono.Cecil.MethodDefinition definition;
        UnityFusion.Runtime.Enviorment.AppDomain appdomain;

        Attribute[] customAttributes;
        Type[] attributeTypes;
        public UnityFusionMethodInfo(ILMethod m)
        {
            method = m;
            definition = m.Definition;
            appdomain = m.DeclearingType.AppDomain;
            parameters = new UnityFusionParameterInfo[m.ParameterCount];
            for (int i = 0; i < m.ParameterCount; i++)
            {
                var pd = m.Definition.Parameters[i];
                parameters[i] = new UnityFusionParameterInfo(pd, m.Parameters[i], this, appdomain);
            }
        }
