    ...
    static class BindingGeneratorExtensions
    {
        ...
        internal static void AppendArgumentCode(this Type p, StringBuilder sb, int idx, string name, List<Type> valueTypeBinders, bool isMultiArr, bool hasByRef, bool needFree)
        {
            ...
            if (pt.IsValueType && !pt.IsPrimitive && valueTypeBinders != null && valueTypeBinders.Contains(pt))
            {
                ...
                if (isMultiArr)
                    sb.AppendLine(string.Format("                UnityFusion.Runtime.Generated.CLRBindings.s_{1}_Binder.ParseValue(ref a{0}, __intp, ptr_of_this_method, __mStack, {2});", idx, clsName, shouldFreeParam));
                else
                    sb.AppendLine(string.Format("                UnityFusion.Runtime.Generated.CLRBindings.s_{1}_Binder.ParseValue(ref @{0}, __intp, ptr_of_this_method, __mStack, {2});", name, clsName, shouldFreeParam));
        ...
        internal static void GetReturnValueCode(this Type type, StringBuilder sb, Enviorment.AppDomain domain)
        {
            ...
            else
            {
                ...
                if (type == typeof(object))
                    ...
                    isBox = "";
                if (!type.IsSealed && type != typeof(UnityFusion.Runtime.Intepreter.ILTypeInstance))
                {
