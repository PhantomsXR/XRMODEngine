...
{
    public class DebugDllLoaderProvider : IDllLoaderProvider
    {
        public async Task LoadAssembly(AppDomain _appDomain, string _projectName)
        {
            ...
            var tmp_PdbBytes =
                await BasePackageLoaderUtility.LoadAssetFromPackage<TextAsset>(_projectName,
                    $"{_projectName.ToLower()}.runtime.pdb");
            SharedData.Instance.symbolStream = new MemoryStream(tmp_PdbBytes.bytes);
            
            _appDomain.LoadAssembly(SharedData.Instance.dllStream, SharedData.Instance.symbolStream,
