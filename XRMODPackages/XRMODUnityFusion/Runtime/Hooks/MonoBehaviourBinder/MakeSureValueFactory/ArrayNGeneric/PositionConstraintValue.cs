...
{
    public class PositionConstraintValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<PositionConstraint>().ToList()
                    : convertedArray.Cast<PositionConstraint>().ToArray());
            return true;
