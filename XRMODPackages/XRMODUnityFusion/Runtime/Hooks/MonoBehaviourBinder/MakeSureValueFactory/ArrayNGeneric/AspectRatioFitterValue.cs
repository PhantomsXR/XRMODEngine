...
{
    public class AspectRatioFitterValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<AspectRatioFitter>().ToList()
                    : convertedArray.Cast<AspectRatioFitter>().ToArray());
            return true;
