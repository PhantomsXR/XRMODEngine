...
{
    public class LightProbeProxyVolumeValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<LightProbeProxyVolume>().ToList()
                    : convertedArray.Cast<LightProbeProxyVolume>().ToArray());
            return true;
