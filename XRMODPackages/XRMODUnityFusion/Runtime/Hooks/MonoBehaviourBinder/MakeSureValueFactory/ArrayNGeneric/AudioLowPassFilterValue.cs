...
{
    public class AudioLowPassFilterValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<AudioLowPassFilter>().ToList()
                    : convertedArray.Cast<AudioLowPassFilter>().ToArray());
            return true;
