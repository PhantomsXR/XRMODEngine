...
{
    public class AudioReverbZoneValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<AudioReverbZone>().ToList()
                    : convertedArray.Cast<AudioReverbZone>().ToArray());
            return true;
