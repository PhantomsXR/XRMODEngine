...
namespace Phantom.XRMOD.UIFramework.Editor
{
    internal class AnimatedPropertiesEditor
    {
        ...
        private AnimatableProperty.ShaderPropertyType _type;
        ...
        public static void DrawAnimatableProperties(SerializedProperty sp, Material[] mats)
        {
            ...
            foreach (var mat in mats)
            {
                ...
                for (var i = 0; i < ShaderUtil.GetPropertyCount(mat.shader); i++)
                {
                    ...
                    AddMenu(gm, sp, new AnimatedPropertiesEditor {_name = pName + "_ST", _type = AnimatableProperty.ShaderPropertyType.Vector}, true);
                    AddMenu(gm, sp, new AnimatedPropertiesEditor {_name = pName + "_HDR", _type = AnimatableProperty.ShaderPropertyType.Vector}, true);
                    AddMenu(gm, sp, new AnimatedPropertiesEditor {_name = pName + "_TexelSize", _type = AnimatableProperty.ShaderPropertyType.Vector}, true);
                }
            }
