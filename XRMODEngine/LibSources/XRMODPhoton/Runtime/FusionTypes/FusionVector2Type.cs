...
{
    public class FusionVector2Type : BaseFusionTypeWithChangeDetector<Vector2>,IGetInterpolatorValue<Vector2>
    {
        ...
        public bool TryGetStateSnapshots(out Vector2 _from,out Tick _fromTick,out Vector2 _to,out Tick _toTick,out float _alpha)
        {
            return base.TryGetStateSnapshots(nameof(Value), out _from, out _fromTick, out _to, out _toTick, out _alpha);
        }
        
        public Vector2 GetInterpolatorValue()
        {
            var tmp_Interpolated = new NetworkBehaviourBufferInterpolator(this);
            return tmp_Interpolated.Vector2(nameof(Value));
        }
    }
}
#endif