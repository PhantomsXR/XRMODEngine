...
{
    public class FusionQuaternionType: BaseFusionTypeWithChangeDetector<Quaternion>,IGetInterpolatorValue<Quaternion>
    {
        ...
        public bool TryGetStateSnapshots(out Quaternion _from,out Tick _fromTick,out Quaternion _to,out Tick _toTick,out float _alpha)
        {
            return base.TryGetStateSnapshots(nameof(Value), out _from, out _fromTick, out _to, out _toTick, out _alpha);
        }
        
        public Quaternion GetInterpolatorValue()
        {
            var tmp_Interpolated = new NetworkBehaviourBufferInterpolator(this);
            return tmp_Interpolated.Quaternion(nameof(Value));
        }
    }
}
#endif