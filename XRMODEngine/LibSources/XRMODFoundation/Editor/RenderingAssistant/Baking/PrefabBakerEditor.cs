using System;
using System.IO;
using Phantom.XRMOD.RenderAssistant.Runtime;
using UnityEditor;
using UnityEngine;
...
namespace Phantom.XRMOD.RenderAssistant.Editor
{
    ...
    public class PrefabBakerEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (hasData)
            {
                ...
                var tmp_ShortPathIndex = Window.folder.IndexOf("Assets", StringComparison.Ordinal);
                var tmp_ShortPath =
                    Window.folder.Substring(tmp_ShortPathIndex, Window.folder.Length - tmp_ShortPathIndex);
                var tmp_LightProbes =
                    AssetDatabase.LoadAssetAtPath<LightProbes>(Path.Combine(tmp_ShortPath, "LightProbes.asset"));
                var tmp_LightProbesProperty = serializedObject.FindProperty("LightProbes");
                if (tmp_LightProbes)
                    tmp_LightProbesProperty.objectReferenceValue = tmp_LightProbes;
                EditorGUILayout.ObjectField(tmp_LightProbesProperty, new GUIContent("Light Probe"));
            ...
            using (var scope = new GUILayout.ScrollViewScope(scroll, GUI.skin.textArea, h))
            {
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    if (Event.current.type == EventType.Repaint)
                        ...
                        scroll.y = (rect.width * 2 + rect.x + 10) > (Screen.width / EditorGUIUtility.pixelsPerPoint)
                            ? -0.05f
                            : 0;
                }
