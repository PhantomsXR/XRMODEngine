...
namespace Phantom.XRMOD.SDKEntry.Runtime
{
    public class SdkVersionCheckHandler : AbstractHandler
    {
        public override void Handle()
        {
            ...
            Assert.IsNotNull(IocContainer.GetIoc.Resolve<RuntimeExperienceConfig>().CurrentConfigures.Value);
            var tmp_ValueVersion = IocContainer.GetIoc.Resolve<RuntimeExperienceConfig>().CurrentConfigures.Value.Version;
            ...
            var tmp_RequiredVersion = IocContainer.GetIoc.Resolve<RuntimeExperienceConfig>().CurrentConfigures
                .Value.Version;
            var tmp_ErrorMsg =
                $"{nameof(ErrorCode.SDK_VERSION_IS_TOO_LOW)},SDK version:{tmp_SdkVersion},AR Experiences required version:{tmp_RequiredVersion}";
            APICallback.ThrowException(tmp_ErrorMsg, (int) ErrorCode.SDK_VERSION_IS_TOO_LOW);
