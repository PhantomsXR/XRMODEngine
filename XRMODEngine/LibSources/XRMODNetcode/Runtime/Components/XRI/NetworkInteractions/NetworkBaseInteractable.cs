...
namespace Phantom.XRMOD.NetcodeModule.Runtime.NetworkInteractions
{
    ...
    public class NetworkBaseInteractable : NetworkBehaviour, IXRSelectFilter, IXRHoverFilter
    {
        ...
        [Header("General Options"), SerializeField,
         Tooltip("Allow users to take ownership of currently controlled objects")]
        ...
        [Header("Ownership Relinquish"), SerializeField,
         Tooltip("Should we relinquish ownership back to the room host after a set amount of time?")]
        ...
        /// </remarks>
        protected NetworkVariable<bool> m_IsInteracting = new(false, NetworkVariableReadPermission.Everyone,
            NetworkVariableWritePermission.Owner);

        [HideInInspector, SerializeField,
         Tooltip("Use a Unity Event for a callback when the IsInteracting value changes.")]
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        /// </summary>
        [HideInInspector, SerializeField] bool[] m_FoldoutValues = {true, true, true};
#endif
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public virtual void OnHoverClientRpc(bool entered, ulong clientId)
        {
            if (clientId != NetworkManager.Singleton.LocalClientId)
            {
                ...
                {
#pragma warning disable CS0618 // Type or member is obsolete
                    m_AffordanceStateProvider.UpdateAffordanceState(
                        new AffordanceStateData(Convert.ToByte(entered ? 2 : (isInteracting ? 4 : 0)), 1.0f));
#pragma warning restore CS0618 // Type or member is obsolete
                }
            }
        }
        ...
        public virtual void OnSelectEnteredLocal(BaseInteractionEventArgs args)
        {
            ...
            if (m_IgnoreSocketSelectedCallback &&
                args.interactorObject.transform.GetComponent<XRSocketInteractor>() != null)
                return;
...
#pragma warning disable CS0618 // Type or member is obsolete
                    m_AffordanceStateProvider.UpdateAffordanceState(
                        new AffordanceStateData(Convert.ToByte(activate ? 5 : isInteracting ? 4 : 0), 1.0f));
#pragma warning restore CS0618 // Type or member is obsolete
                }
            }
        }
        ...
        IEnumerator CheckForOwnerInteraction()
        {
            while (IsOwner)
            {
                if (isInteracting)
                {
                    ...
                    {
                        Debug.Log(
                            $"Interacting is true and either selected is false or selected is a socket on object {gameObject.name}. Is this intentional?");
                        m_IsInteracting.Value = false;
                    }
                }
...
#pragma warning disable CS0618 // Type or member is obsolete
                m_AffordanceStateProvider.UpdateAffordanceState(
                    new AffordanceStateData(Convert.ToByte(newValue ? 4 : 0), 1.0f));
#pragma warning restore CS0618 // Type or member is obsolete
            }
