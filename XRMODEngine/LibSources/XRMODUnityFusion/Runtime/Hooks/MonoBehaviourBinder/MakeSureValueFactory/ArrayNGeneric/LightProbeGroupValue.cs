...
{
    public class LightProbeGroupValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<LightProbeGroup>().ToList()
                    : convertedArray.Cast<LightProbeGroup>().ToArray());
            return true;
