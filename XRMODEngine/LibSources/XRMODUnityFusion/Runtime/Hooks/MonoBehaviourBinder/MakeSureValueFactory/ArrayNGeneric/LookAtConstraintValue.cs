...
{
    public class LookAtConstraintValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<LookAtConstraint>().ToList()
                    : convertedArray.Cast<LookAtConstraint>().ToArray());
            return true;
