...
{
    public class TerrainColliderValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<TerrainCollider>().ToList()
                    : convertedArray.Cast<TerrainCollider>().ToArray());
            return true;
