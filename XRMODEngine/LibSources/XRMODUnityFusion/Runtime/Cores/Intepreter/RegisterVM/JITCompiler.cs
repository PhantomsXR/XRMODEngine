...
namespace UnityFusion.Runtime.Intepreter.RegisterVM
{
    struct JITCompiler
    {
        ...
        int InitializeFunctionParam(ref OpCodes.OpCodeR op, object token, out bool hasReturn, out bool canInline, out IMethod m, out ILMethod toInline, out bool isILMethod)
        {
            ...
            if (m != null)
            {
                ...
                if (m is ILMethod)
                {
                    ...
                    var ilm = (ILMethod)m;
                    bool noJIT = (ilm.JITFlags & UnityFusionJITFlags.NoJIT) != UnityFusionJITFlags.None;
                    bool forceInline = (ilm.JITFlags & UnityFusionJITFlags.ForceInline) != UnityFusionJITFlags.None;
