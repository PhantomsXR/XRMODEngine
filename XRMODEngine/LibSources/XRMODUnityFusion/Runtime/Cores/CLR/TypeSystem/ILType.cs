...
namespace UnityFusion.CLR.TypeSystem
{
    public sealed class ILType : IType
    {
        ...
        bool isDelegate;
        UnityFusionType reflectionType;
        ...
        {
            get { return typeRef.HasGenericParameters && genericArguments == null; }
        }

        public bool IsGenericParameter
        {
            get { return typeRef.IsGenericParameter && genericArguments == null; }
        }

        public Dictionary<string, int> StaticFieldMapping
        {
            get { return staticFieldMapping; }
        }
        ...
        public bool IsByRef
        {
            get { return typeRef.IsByReference; }
        }
        ...
        {
            get { return TypeDefinition.IsInterface; }
        }
        ...
        public Type ReflectionType
        {
            get
            {
                if (reflectionType == null)
                    reflectionType = new UnityFusionType(this);
        ...
        {
            get { return arrayTypes != null ? arrayTypes[1] : null; }
        }

        public bool IsEnum
        {
            get { return definition != null ? definition.IsEnum : false; }
        }
        ...
        public string Name
        {
            get { return typeRef.Name; }
        }

        public StackObject DefaultObject
        {
            get { return default(StackObject); }
        }
                    ...
                    if (interfaces[i] is CLRType && firstCLRInterface == null)
                    {
                        ...
                        if (appdomain.CrossBindingAdaptors.TryGetValue(interfaces[i].TypeForCLR, out adaptor))
                        {
                            ...
                            throw new TypeLoadException(
                                "Cannot find Adaptor for:" + interfaces[i].TypeForCLR.ToString());
                    }
                }
            }
        ...
        void InitializeBaseType()
        {
            if (definition != null && definition.BaseType != null)
            {
                ...
                if (specialProcess)
                {
                    ...
                    var elementType = appdomain.GetType(((GenericInstanceType) definition.BaseType).ElementType, this,
                        null);
                ...
                else
                {
                    ...
                    {
                        if (baseType.TypeForCLR == typeof(Enum) || baseType.TypeForCLR == typeof(object) ||
                            baseType.TypeForCLR == typeof(ValueType) || baseType.TypeForCLR == typeof(System.Enum))
                        {
                            //都是这样，无所谓
                            baseType = null;
                        }
        ...
        public IMethod GetVirtualMethod(IMethod method)
        {
            ...
            if (m == null && method.DeclearingType.IsInterface)
            {
                if (method.DeclearingType is ILType)
                {
                    ...
                    m = GetMethod(string.Format("{0}.{1}", iltype.FullNameForNested, method.Name), method.Parameters,
                        genericArguments, method.ReturnType, true);
                }
                else
                    m = GetMethod(string.Format("{0}.{1}", method.DeclearingType.FullName, method.Name),
                        method.Parameters, genericArguments, method.ReturnType, true);
            }
        ...
        ILMethod CheckGenericParams(ILMethod i, List<IType> param, IType[] genericArguments, ref bool match)
        {
            ...
            if (param != null)
            {
                for (int j = 0; j < param.Count; j++)
                {
                    ...
                    if (p.IsGenericParameter)
                    {
                        if (i.Parameters[j].IsByRef == param[j].IsByRef &&
                            i.Parameters[j].IsArray == param[j].IsArray &&
                            IsGenericArgumentMatch(p, p2, genericArguments))
                            continue;
                        else
                        {
                            match = false;
        ...
        public IType MakeArrayType(int rank)
        {
            ...
            if (!arrayTypes.TryGetValue(rank, out atype))
            {
                ...
                ((ILType) atype).arrayCLRType =
                    rank > 1 ? this.TypeForCLR.MakeArrayType(rank) : this.TypeForCLR.MakeArrayType();
