    ...
    public class AgentTypeJobRunner : IAgentTypeJobRunner
    {
        ...
        private void FillBuilders(IMonoGoapActionProvider actionProvider)
        {
            try
            {
                ...
                {
                    this.costBuilder.SetCost(goal,
                        goal.GetCost(actionProvider.Receiver, actionProvider.Receiver.Injector));
                }
        ...
        public void Complete()
        {
            foreach (var resolveHandle in this.resolveHandles)
            {
                ...
                {
                    var tmp_GoalResult = new GoalResult
                    {
                        Goal = goal,
                        Plan = result.Actions,
                        Action = action,
                    };
                    resolveHandle.ActionProvider.SetAction(tmp_GoalResult);
                }
            }
        ...
        private void LogRequest(IGoapActionProvider actionProvider, IGoalRequest request)
        {
            ...
            {
                builder.Append(TypeReSolveHelper.ResolveType(goal)?.GetGenericTypeName());
                builder.Append(", ");
            }
