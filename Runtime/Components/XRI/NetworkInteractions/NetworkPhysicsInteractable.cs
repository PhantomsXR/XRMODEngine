...
namespace Phantom.XRMOD.NetcodeModule.Runtime.NetworkInteractions
{
    ...
    public class NetworkPhysicsInteractable : NetworkBaseInteractable
    {
        ...
        /// </summary>
        [Header("Collision Based Ownership Transfer"), SerializeField,
         Tooltip("Enabling this will call Request Ownership on collision with non-local NetworkPhysicsInteractables.")]
        ...
        /// </summary>
        [SerializeField,
         Tooltip("Determines the minimum velocity magnitude required to request ownership on collision.")]
        ...
        /// </remarks>
        [Header("Spawn Options")] public bool spawnLocked = true;
        ...
        protected NetworkVariable<bool> m_LockedOnSpawn = new(true, NetworkVariableReadPermission.Everyone,
        ...
        /// </summary>
        protected NetworkVariable<bool> m_ResettingObject = new(false, NetworkVariableReadPermission.Everyone,
            NetworkVariableWritePermission.Owner);
        ...
        public override void OnSelectEnteredLocal(BaseInteractionEventArgs args)
        {
            ...
            if (m_IgnoreSocketSelectedCallback &&
                args.interactorObject.transform.GetComponent<XRSocketInteractor>() != null) return;
        ...
        public override void OnSelectExitedLocal(BaseInteractionEventArgs args)
        {
            ...
            if (IsOwner)
            {
                ...
                {
                    if (((XRGrabInteractable) baseInteractable).movementType ==
                        XRBaseInteractable.MovementType.VelocityTracking ||
                        ((UnityEngine.XR.Interaction.Toolkit.Interactables.XRGrabInteractable) baseInteractable)
                        .throwOnDetach)
                    {
        ...
        void OnCollisionEnter(Collision collision)
        {
            ...
            NetworkPhysicsInteractable networkPhysicsInteractable =
                collision.transform.GetComponentInParent<NetworkPhysicsInteractable>();
            if (networkPhysicsInteractable != null &&
                (isInteracting || IsMovingFaster(networkPhysicsInteractable.m_Rigidbody)))
            {
        ...
        {
            return m_Rigidbody.linearVelocity.magnitude > m_MinExchangeVelocityMagitude &&
                   m_Rigidbody.linearVelocity.magnitude > otherBody.linearVelocity.magnitude;
        }
        ...
        {
            return isInteracting || IsOwner || m_RequestingOwnership || m_LockedOnSpawn.Value ||
                   !m_AllowCollisionOwnershipExchange || !NetworkObject.IsSpawned || baseInteractable.isSelected ||
                   m_ResettingObject.Value;
        }
