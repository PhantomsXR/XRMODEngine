...
{
    public class BillboardRendererValue:ArrayAndGenericMixMakeSureAction
    {
        public override bool MakeSureValue(ref FieldInfo _fieldInfo, object _target, object _value)
        {
            ...
            _fieldInfo.SetValue(_target,
                isGenericList
                    ? convertedArray.Cast<BillboardRenderer>().ToList()
                    : convertedArray.Cast<BillboardRenderer>().ToArray());
            return true;
