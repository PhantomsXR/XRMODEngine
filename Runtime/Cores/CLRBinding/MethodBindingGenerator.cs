            ...
            foreach (var i in methods)
            {
                ...
                if (!i.IsStatic)
                {
                    ...
                    else if (type.IsValueType && !type.IsPrimitive && valueTypeBinders != null && valueTypeBinders.Contains(type))
                    {
                        ...
                        sb.AppendLine(string.Format("                UnityFusion.Runtime.Generated.CLRBindings.s_{0}_Binder.ParseValue(ref instance_of_this_method, __intp, ptr_of_this_method, __mStack, false);", clsName));
                ...
                case ObjectTypes.StackObjectReference:
                    {
                        ...
                        if (pt.IsValueType && !pt.IsPrimitive && valueTypeBinders != null && valueTypeBinders.Contains(pt))
                        {
                            ...
                            sb.AppendLine(string.Format("                        UnityFusion.Runtime.Generated.CLRBindings.s_{0}_Binder.WriteBackValue(__domain, ptr_of_this_method, __mStack, ref {1});", clsName, p.Name));
                ...
                if (!i.IsStatic && ((type.IsValueType && !type.IsPrimitive) || hasByRef))//need to write back value type instance
                {
                    ...
                    if (type.IsValueType && !type.IsPrimitive && !noWriteback)
                    {
                        if (valueTypeBinders != null && valueTypeBinders.Contains(type))
                        {
                            ...
                            sb.AppendLine(string.Format("                UnityFusion.Runtime.Generated.CLRBindings.s_{0}_Binder.WriteBackValue(__domain, ptr_of_this_method, __mStack, ref instance_of_this_method);", clsName));
