...
namespace Goap.Goap.Editor
{
    ...
    public class GoapActionProviderEditor : UnityEditor.Editor
    {
        ...
        {
            var requestGoals = provider.GoalRequest?.Goals?.Select(x => TypeReSolveHelper.ResolveType(x)?.GetGenericTypeName());
            
            return $@"Goal: {TypeReSolveHelper.ResolveType(provider.CurrentPlan?.Goal)?.GetGenericTypeName()}
Request: {string.Join(", ", requestGoals ?? Array.Empty<string>())}
AgentType: {provider.AgentType?.Id}
Receiver: {TypeReSolveHelper.ResolveType(provider.Receiver)?.GetGenericTypeName()}";
        }
    }
}