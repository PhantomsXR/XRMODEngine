...
using Phantom.XRMOD.Localization.Editor;
...
namespace Phantom.XRMOD.XRMODPackageTools.Editor
{
    public class CreateProjectCommand : BaseGenericMenuItemCommand
    {
        ...
        private const string CONST_SCRIPT_TEMPLATE_NAME = "ScriptTemplates/EntryTemplate.txt";
        ...
        private static readonly string CONST_TEMPLATES_PATH = $"{ConstKey.CONST_PACKAGE_PATH}/Editor/Templates";
        private const string CONST_TEMPLATE_TEMP_PATH = "Editor Default Resources/PackageToolsCache";
            ...
            string _editorFolderPath)
        {
            ...
            GenerationASMDEF(_projectName, CONST_RUNTIME_NAME, _runtimeFolderPath, new List<string>(),
                new List<string>
                {
                    ...
                    ConstKey.CONST_XRMODCORE,
                    ConstKey.CONST_XRMODINPUT,
                    ConstKey.ACTION_NOTIFICATION,
                    ConstKey.CONST_XRMODUTILITIES,
                    ConstKey.CONST_LOCALIZATION,
                    ConstKey.CONST_XRMOD_LITJSON,

                    ConstKey.CONST_XRMOD_GAMESERVICES,
                    ConstKey.CONST_LEAN_TWEEN,

                    ConstKey.CONST_UNITY_XRI,
                    ConstKey.CONST_UNITY_HAND,
                    ConstKey.CONST_TEXT_MESH_PRO,
                    ConstKey.CONST_UNITY_INPUTSYSTEM,
                });
            ...
            GenerateMainEntryScript(_runtimeFolderPath, _projectName,
                $"{_projectName}{CONST_MAIN_ENTRY_NAME}");

            GenerateOtherScripts(_runtimeFolderPath, _projectName,
                "SharedData",
                "SharedData.txt");
            
            GenerateOtherScripts(_runtimeFolderPath, _projectName,
                "BindableProperty",
                "BindableProperty.txt");
            
            GenerateOtherScripts(_runtimeFolderPath, _projectName,
                "UnityFusionEventBus",
                "UnityFusionEventBusTemplate.txt");
            
            GenerateOtherScripts(_runtimeFolderPath, _projectName,
                "UnityFusionSingletonTemplate",
                "UnityFusionSingletonTemplate.txt");
        }
            ...
            out string _editorFolderPath)
        {
            ...
            var tmp_LocalizationFilePath =
                Path.Combine(Utility.ShortenPath(_projectPath), "Artwork/LocalizationDatabase.asset");
            LocalizationDbEditor.CreateDbAsset(_path: tmp_LocalizationFilePath,
                _scope: LocalizationScope.InExperiences);
        ...
        private void GenerateMainEntryScript(string _destFilePath, string _namespace,
            string _scriptName)
        {
            var tmp_BaseDir = $"{ConstKey.CONST_PACKAGE_PATH}/{CONST_EDITOR_NAME}/PackageTools/";
        ...
        /// <param name="_namespace">Script's namespace</param>
        /// <param name="_scriptName">Script name(file name and class name)</param>
        /// <param name="_templateName">The template script path</param>
        private void GenerateOtherScripts(string _destFilePath, string _namespace, string _scriptName,
            string _templateName)
        {
            var tmp_BaseDir = $"{ConstKey.CONST_PACKAGE_PATH}/Editor/PackageTools/ScriptTemplates/";
            var tmp_TemplateScriptPath = Path.GetFullPath($"{tmp_BaseDir}/{_templateName}");
            var tmp_TemplateScriptText = File.ReadAllText(tmp_TemplateScriptPath);
            tmp_TemplateScriptText = tmp_TemplateScriptText.Replace(CONST_NAME_SPACE_NAME, _namespace);
            File.WriteAllText(Path.Combine(_destFilePath, _scriptName + ".cs"), tmp_TemplateScriptText, Encoding.UTF8);
        }
        ...
        private static void ImportPackageCompletedCallback(string _templateName)
        {
            ...
            {
                var tmp_Key = $"{Application.productName}{CONST_REGISTER_IMPORT_EVENT_NAME}";
                if (!EditorPrefs.GetBool(tmp_Key, false)) return;
                EditorPrefs.DeleteKey(tmp_Key);
                try
                {
                    ...
                    if (!tmp_TemplateDirectory.Exists) return;
